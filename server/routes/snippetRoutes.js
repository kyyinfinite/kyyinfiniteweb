...
router.get('/:id', async (req, res) => {
  try {
    const snippet = await Snippet.findById(req.params.id).populate('forkedFrom', 'title').lean();
...
router.put('/:id', adminAuthMiddleware, async (req, res) => {
  try {
    const before = await Snippet.findById(req.params.id).select('status source ownerUid title').lean();
    const snippet = await Snippet.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!snippet) {
      return res.status(404).json({ message: 'Snippet not found' });
    }

    const statusChanged = req.body?.status && before && before.status !== req.body.status;
    if (statusChanged && snippet.source === 'community' && snippet.ownerUid) {
      if (snippet.status === 'approved') {
        dispatchEvent('snippet.approved', { snippetId: snippet._id, title: snippet.title }, { ownerUid: snippet.ownerUid });
      } else if (snippet.status === 'rejected') {
        dispatchEvent('snippet.rejected', { snippetId: snippet._id, title: snippet.title }, { ownerUid: snippet.ownerUid });
      }
    }
