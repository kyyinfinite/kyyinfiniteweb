...
const userWebhookRoutes = require('./routes/userWebhookRoutes');
const apiKeySnippetRoutes = require('./routes/apiKeySnippetRoutes');
const alightMotionRoutes = require('./routes/alightMotionRoutes');
...
app.get('/raw/:id', async (req, res) => {
  try {
    const snippet = await Snippet.findById(req.params.id).select('code isPublished status').lean();
...
app.use('/api/v1/account/snippets', apiKeySnippetRoutes);
app.use('/api/v1/am', alightMotionRoutes);
